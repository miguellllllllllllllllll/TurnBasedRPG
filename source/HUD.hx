...
class HUD extends FlxTypedGroup<FlxSprite>
{
	...
	public function new()
	{
		...
		healthIcon = new FlxSprite(4, healthCounter.y + (healthCounter.height/2)  - 7, AssetPaths.health__png);
		moneyIcon = new FlxSprite(FlxG.width - 20, moneyCounter.y + (moneyCounter.height/2) - 8, AssetPaths.coin__png);
        charIcon = new FlxSprite(5,25,AssetPaths.gokuportrait__png);
		moneyCounter.alignment = RIGHT;
		moneyCounter.x = moneyIcon.x - moneyCounter.width;
		add(background);
		add(healthIcon);
		add(moneyIcon);
		add(healthCounter);
	...
	public function updateHUD(health:Int, money:Int)
	{
		...
		moneyCounter.x = moneyIcon.x - moneyCounter.width - 7 ;
	}
}